...
function loadParameters() {
  ...
  if (savedResolution !== null) {
    ...
    const optionExists = Array.from(resolutionSelect.options).some(
      (opt) => parseFloat(opt.value) === resolution,
    );
...
function applyModelRotation(axis, direction) {
  ...
  cacheSTL(
    'model-stl',
    modelSTL,
    document.getElementById('model-status').textContent || 'model.stl',
  ).catch((err) => debug.warn('Failed to cache rotated model:', err));

  updateButtonStates();
  updateInfo(
    `Model rotated ${direction > 0 ? '+' : ''}${direction * 90}° around ${axis.toUpperCase()}`,
  );
}
...
async function loadSTLFile(isModel) {
  ...
  return new Promise((resolve) => {
    input.onchange = async (e) => {
      ...
      updateInfo(
        `Loaded ${file.name}: ${(triangles.length / 9).toLocaleString()} triangles`,
      );
...
async function initRasterPath() {
  ...
  const radialV3Checkbox = document.getElementById('radial-v3');
  const useRadialV3 =
    mode === 'radial' && radialV3Checkbox && radialV3Checkbox.checked;
  ...
  updateInfo(
    `RasterPath initialized: ${mode} mode, ${resolution}mm resolution`,
  );
}
  ...
  try {
    ...
    if (mode === 'planar' || mode === 'tracing') {
      ...
      if (modelTriangles) {
        ...
        const t1 = performance.now();
        updateInfo(
          `Terrain loaded in ${(t1 - t0).toFixed(0)}ms (rasterized in toolpath generation)`,
        );
      } else {
        updateInfo(
          'Tool loaded. Load model and click "Generate Toolpath" to continue.',
        );
      }
...
async function generateToolpath() {
  ...
  try {
    ...
    if (mode === 'tracing') {
      ...
      debug.log(
        `Generated trace paths: H(${minX.toFixed(2)}, ${centerY.toFixed(2)}) to (${maxX.toFixed(2)}, ${centerY.toFixed(2)})`,
      );
      debug.log(
        `                        V(${centerX.toFixed(2)}, ${minY.toFixed(2)}) to (${centerX.toFixed(2)}, ${maxY.toFixed(2)})`,
      );
    }
    ...
    if (mode === 'planar') {
      ...
      updateInfo(
        `Toolpath generated: ${numPoints.toLocaleString()} points in ${(t1 - t0).toFixed(0)}ms`,
      );
    } else if (mode === 'tracing') {
      const totalPoints = toolpathData.paths.reduce(
        (sum, path) => sum + path.length / 3,
        0,
      );
      debug.log(
        `[Tracing] Generated ${toolpathData.paths.length} paths with ${totalPoints} total points`,
      );
      ...
      updateInfo(
        `Toolpath generated: ${toolpathData.paths.length} paths, ${totalPoints.toLocaleString()} points in ${(t1 - t0).toFixed(0)}ms`,
      );
    } else {
      // debug.log('[Radial] Toolpaths generated:', toolpathData);
      debug.log(
        `[Radial] Received ${toolpathData.strips.length} strips from worker, numStrips=${toolpathData.numStrips}`,
      );
      updateInfo(
        `Toolpath generated: ${toolpathData.numStrips} strips, ${toolpathData.totalPoints.toLocaleString()} points in ${(t1 - t0).toFixed(0)}ms`,
      );
      ...
      if (toolpathData.terrainStrips) {
        ...
        debug.log(
          '[Radial] Terrain strips available:',
          modelRasterData.length,
          'strips',
        );
        ...
        if (modelRasterData.length > 0) {
          ...
          if (strip0.positions && strip0.positions.length > 0) {
            ...
            debug.log(
              '[Radial] Strip 0 actual X range in positions:',
              minX.toFixed(2),
              'to',
              maxX.toFixed(2),
            );
        ...
        debug.log(
          '[Radial] Batched mode: created synthetic terrain strips from bounds:',
          modelRasterData.length,
          'strips',
        );
      }
...
function displayModelMesh() {
  ...
  geometry.setAttribute(
    'position',
    new THREE.BufferAttribute(modelTriangles, 3),
  );
  geometry.computeVertexNormals();
  geometry.computeBoundingBox();
...
function displayToolMesh() {
  ...
  geometry.setAttribute(
    'position',
    new THREE.BufferAttribute(toolTriangles, 3),
  );
  geometry.computeVertexNormals();
...
function displayModelRaster(wrapped) {
  ...
  if (mode === 'planar' || mode === 'tracing') {
    ...
    if (wrapped) {
      ...
      debug.log(
        '[Display] X range: [' +
          overallMinX.toFixed(2) +
          ', ' +
          overallMaxX.toFixed(2) +
          ']',
      );
      ...
      debug.log(
        '[Display] Angle range: [' +
          angles[0].toFixed(1) +
          '°, ' +
          angles[angles.length - 1].toFixed(1) +
          '°]',
      );
      debug.log(
        '[Display] First 5 angles:',
        angles
          .slice(0, 5)
          .map((a) => a.toFixed(1) + '°')
          .join(', '),
...
function displayToolpaths(wrapped) {
  ...
  if (mode === 'planar') {
    ...
    if (totalPoints > MAX_DISPLAY_POINTS) {
      debug.warn(
        `[Toolpath Display] Toolpath too large for visualization: ${(totalPoints / 1e6).toFixed(1)}M points`,
      );
      debug.warn(
        `[Toolpath Display] Skipping display (max: ${(MAX_DISPLAY_POINTS / 1e6).toFixed(1)}M points)`,
      );
      debug.warn(
        `[Toolpath Display] Toolpath was generated successfully - only visualization is skipped`,
      );
      return;
    }
  ...
  debug.log(
    '[Toolpath Display] Radial V2 mode:',
    strips.length,
    'strips,',
    totalPoints,
    'total points',
  ...
  if (strips.length > 0) {
    const angleChecks = [0, 180, 359, 360, 361, 540, 719].filter(
      (i) => i < strips.length,
    );
    debug.log(
      '[Toolpath Display] Angle check at indices:',
      angleChecks.map((i) => `${i}=${strips[i].angle.toFixed(1)}°`).join(', '),
    );
    // Check if pathData is actually different between strips
    if (strips.length > 360) {
      const samples0 = strips[0].pathData
        .slice(0, 5)
        .map((v) => v.toFixed(3))
        .join(',');
      const samples360 = strips[360].pathData
        .slice(0, 5)
        .map((v) => v.toFixed(3))
        .join(',');
      debug.log(
        '[Toolpath Display] Data check: strip 0 first 5 values:',
        samples0,
      );
      debug.log(
        '[Toolpath Display] Data check: strip 360 first 5 values:',
        samples360,
      );
      debug.log(
        '[Toolpath Display] Data is',
        samples0 === samples360 ? 'SAME (BUG!)' : 'DIFFERENT (OK)',
      );
    }
  }
  ...
  if (strips.length > 0) {
    ...
    const terrainStrip = modelRasterData
      ? modelRasterData.find((s) => s.angle === firstStrip.angle)
      : null;
  ...
  if (wrapped) {
    ...
    for (const strip of strips) {
      ...
      const stripBounds = terrainStrip?.bounds || {
        min: { x: -100, y: 0, z: 0 },
        max: { x: 100, y: 10, z: 20 },
      };
    ...
    for (const strip of strips) {
      ...
      const stripBounds = (modelRasterData &&
        modelRasterData.find((s) => s.angle === angle)?.bounds) || {
        min: { x: -100, y: 0, z: 0 },
        max: { x: 100, y: 10, z: 20 },
      };
...
function updateModeUI() {
  ...
  const wrappedContainer =
    document.getElementById('wrapped-container').classList;
  const angleStepContainer = document.getElementById(
    'angle-step-container',
  ).classList;
  const traceStepContainer = document.getElementById(
  ...
  const radialV3Container = document.getElementById(
    'radial-v3-container',
  ).classList;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (cachedModel) {
    ...
    document.getElementById('model-status').textContent = isOldFormat
      ? 'Cached model'
      : cachedModel.name || 'Cached model';
    displayModelMesh();
  }
  ...
  if (cachedTool) {
    ...
    // Update status
    document.getElementById('tool-status').textContent = isOldFormat
      ? 'Cached tool'
      : cachedTool.name || 'Cached tool';
    ...
    if (toolOriginalTriangles) {
      ...
      if (rasterPath && wasToolLoaded) {
        ...
        try {
          ...
          const t1 = performance.now();
          updateInfo(
            `Tool reloaded at ${toolSize}mm in ${(t1 - t0).toFixed(0)}ms`,
          );
        } catch (error) {
        ...
        updateInfo(
          `Tool size changed to ${toolSize}mm - click Rasterize to apply`,
        );
      }
  ...
  document
    ...
    .addEventListener('click', resetModelRotation);
  ...
  document
    .getElementById('generate-toolpath')
    .addEventListener('click', generateToolpath);
